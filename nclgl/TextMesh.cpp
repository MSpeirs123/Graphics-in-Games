...
TextMesh::TextMesh(const std::string &text, const Font &font) :  font(font)	{
	...
	*/
	for(unsigned int i = 0; i < text.length(); ++i) {
		...
		verticies[(i*4)  ] = Vector3((float)i  ,  0,0);
		verticies[(i*4)+1] = Vector3((float)i  , -1,0);
		verticies[(i*4)+2] = Vector3((float)i+1,  0,0);
		verticies[(i*4)+3] = Vector3((float)i+1, -1,0);
