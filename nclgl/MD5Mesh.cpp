...
void MD5Mesh::BufferExtraData() {
	...
	glBufferData(GL_ARRAY_BUFFER, numVerticies *sizeof(Vector2), weights, GL_STATIC_DRAW);
	glVertexAttribPointer(6, 2, GL_FLOAT, GL_FALSE, 0, 0); //MAX_BUFFER
	glEnableVertexAttribArray(6); //MAX_BUFFER
			...
			*/
			...
			for(int k = 0; k < subMesh.verts[j].weightElements; ++k) {
				...
				*/
				...
				target->verticies[j] += ((joint.transform * weight.position) * weight.weightValue);
			}
		}
...
void MD5Mesh::RebufferData()	{
	...
	glBufferSubData(GL_ARRAY_BUFFER, 0, numVerticies *sizeof(Vector3), (void*)verticies);

	if(textureCoords) {
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TEXTURE_BUFFER]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVerticies *sizeof(Vector2), (void*)textureCoords);
	}

	if (colours)	{
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[COLOUR_BUFFER]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVerticies *sizeof(Vector4), (void*)colours);
	}
	...
	if(indicies) {
		...
		glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, numIndicies *sizeof(unsigned int), (void*)indicies);
	}
}
#endif
#endif