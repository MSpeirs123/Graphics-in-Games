...
Particle* ParticleEmitter::GetFreeParticle()	{
	...
	p->direction.y += (p->direction.x * p->direction.x + p->direction.z * p->direction.z);
...
void	ParticleEmitter::ResizeArrays() {
	...
	verticies = new Vector3[particles.size()];
...
void ParticleEmitter::Draw()	{
	...
	for(unsigned int i = 0; i < particles.size(); ++i) {
		verticies[i] = particles.at(i)->position;
	...
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[VERTEX_BUFFER]);
	glBufferSubData(GL_ARRAY_BUFFER, 0, particles.size()*sizeof(Vector3), (void*)verticies);
